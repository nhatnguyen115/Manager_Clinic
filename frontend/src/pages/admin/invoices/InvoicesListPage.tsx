        ...
        <Modal isOpen={isOpen} onClose={onClose} title={`Hóa đơn ${invoice.invoiceNumber}`} size="lg">
            <div className="space-y-5">
                ...
                <div className="flex items-center gap-3">
                    ...
                    <span className="text-xs text-dark-500 font-mono">{invoice.invoiceNumber}</span>
                </div>

                {/* Info Grid */}
                <div className="grid grid-cols-2 gap-3">
                    {invoice.patientName && (
                        <div className="flex items-center gap-2 p-3 rounded-xl bg-dark-800/40 border border-dark-700/30">
                            <User size={16} className="text-blue-400 flex-shrink-0" />
                            <div className="min-w-0">
                                <p className="text-xs text-dark-500">Bệnh nhân</p>
                                <p className="text-sm font-semibold text-dark-100 truncate">{invoice.patientName}</p>
                            </div>
                        </div>
                    )}
                    {invoice.doctorName && (
                        <div className="flex items-center gap-2 p-3 rounded-xl bg-dark-800/40 border border-dark-700/30">
                            ...
                            <div className="min-w-0">
                                <p className="text-xs text-dark-500">Bác sĩ</p>
                                <p className="text-sm font-semibold text-dark-100 truncate">{invoice.doctorName}</p>
                            </div>
                        </div>
                    )}
                    ...
                    <div className="flex items-center gap-2 p-3 rounded-xl bg-dark-800/40 border border-dark-700/30">
                        ...
                        <div className="min-w-0">
                            <p className="text-xs text-dark-500">Ngày xuất</p>
                            <p className="text-sm font-semibold text-dark-100">
                ...
                {/* Items */}
                <div>
                    <h3 className="text-xs font-bold text-dark-400 uppercase tracking-wider mb-2">Dịch vụ</h3>
                    <div className="rounded-xl overflow-hidden border border-dark-700/30">
                        <table className="w-full text-left text-sm">
                            <thead className="bg-dark-800/50">
                                <tr>
                                    <th className="px-3 py-2 text-xs text-dark-400">Mô tả</th>
                                    <th className="px-3 py-2 text-xs text-dark-400 text-right">Thành tiền</th>
                                </tr>
                            </thead>
                            <tbody>
                                {items.length > 0 ? items.map((item: any, i: number) => (
                                    <tr key={i} className="border-t border-dark-700/30">
                                        <td className="px-3 py-2 text-dark-200">{item.description}</td>
                                        <td className="px-3 py-2 text-dark-100 font-semibold text-right">
                                            {Number(item.total).toLocaleString('vi-VN')} đ
                                        </td>
                                    </tr>
                                )) : (
                                    <tr>
                                        <td className="px-3 py-2 text-dark-400">Phí khám bệnh</td>
                                        <td className="px-3 py-2 text-dark-100 font-semibold text-right">
                ...
                <div className="flex justify-between items-center p-4 bg-primary-900/10 border border-primary-500/20 rounded-xl">
                    <span className="font-bold text-dark-200">Tổng cộng</span>
                ...
                {!isPaid && (
                    <div className="border-t border-dark-700/30 pt-4">
                        <p className="text-sm font-semibold text-dark-300 mb-3">Xác nhận thanh toán thủ công</p>
                        <div className="flex gap-3">
                            <select
                                value={selectedMethod}
                                onChange={(e) => setSelectedMethod(e.target.value)}
                                className="flex-1 bg-dark-800 border border-dark-700/50 rounded-xl px-3 py-2 text-dark-100 focus:outline-none focus:ring-2 focus:ring-primary-500/50"
                            >
    ...
    return (
        <div className="space-y-6 animate-fade-in">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <h1 className="text-3xl font-black text-dark-50 tracking-tight">Quản lý hóa đơn</h1>
                    <p className="text-dark-400 mt-1">Tổng cộng {invoices.length} hóa đơn đã phát hành</p>
                </div>
            </div>
            ...
            {/* Table */}
            <Card className="border-dark-700/30 overflow-hidden">
                <div className="overflow-x-auto">
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="bg-dark-800/50 border-b border-dark-700/30">
                                <th className="px-6 py-4 text-sm font-bold text-dark-400 uppercase tracking-wider">Mã Hóa Đơn</th>
                                <th className="px-6 py-4 text-sm font-bold text-dark-400 uppercase tracking-wider">Bệnh nhân</th>
                                <th className="px-6 py-4 text-sm font-bold text-dark-400 uppercase tracking-wider">Bác sĩ</th>
                                <th className="px-6 py-4 text-sm font-bold text-dark-400 uppercase tracking-wider">Ngày xuất</th>
                                <th className="px-6 py-4 text-sm font-bold text-dark-400 uppercase tracking-wider">Tổng tiền</th>
                                <th className="px-6 py-4 text-sm font-bold text-dark-400 uppercase tracking-wider">Trạng thái</th>
                                <th className="px-6 py-4 text-sm font-bold text-dark-400 uppercase tracking-wider text-right">Thao tác</th>
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-dark-700/30">
                            {filteredInvoices.length > 0 ? (
                                filteredInvoices.map((invoice) => (
                                    <tr key={invoice.id} className="hover:bg-dark-800/30 transition-colors">
                                        <td className="px-6 py-4">
                                            <div className="flex items-center gap-3">
                                                <div className="w-10 h-10 rounded-lg bg-primary-500/10 flex items-center justify-center text-primary-400">
                                                    <Receipt size={18} />
                                                </div>
                                                <span className="font-bold text-dark-100 font-mono text-sm">{invoice.invoiceNumber}</span>
                                            </div>
                                        </td>
                                        <td className="px-6 py-4 text-dark-300 text-sm">{invoice.patientName || '—'}</td>
                                        <td className="px-6 py-4 text-dark-300 text-sm">{invoice.doctorName || '—'}</td>
                                        ...
                                        <td className="px-6 py-4 text-right">
                                            <Button
                                                variant="ghost"
                                                size="sm"
                                                className="h-8 text-dark-400 hover:text-primary-400"
